    ...
    function draw() {
        ...
        stars.forEach((star) => {
            const twinkle = Math.sin(
                time * star.twinkleSpeed * 60 + star.twinklePhase,
            );
...
function initContactForm() {
    ...
    form.addEventListener("submit", (e) => {
        ...
        btn.innerHTML =
            '<span style="display:flex;align-items:center;gap:8px;justify-content:center;">✓ Message Sent!</span>';
...
// ===== MOCKUP SLIDESHOW =====
function initMockupSlideshow() {
    const slideshows = document.querySelectorAll(".browser-mockup");
    
    slideshows.forEach(slideshow => {
        const slides = slideshow.querySelectorAll(".mockup-slide");
        if (slides.length <= 1) return; // Only animate if there are multiple slides
        ...
        setInterval(() => {
            ...
            // Add active to new slide to trigger crossfade and animations
            slides[currentSlide].classList.add("active");

            // Create icons in case new DOM elements became visible
            if (window.lucide) {
